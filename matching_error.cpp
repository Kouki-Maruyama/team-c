...
Point SSDA_R(unsigned char input_g[INPUT_SIZE_H][INPUT_SIZE_W], unsigned char temp_g[TMP_SIZE_H][TMP_SIZE_W]){
    ...
    for(j = 0; j < loss_SIZE_H; j++){
        for(i = 0; i < loss_SIZE_W; i++){
            ...
            //|| input_g[j][i+TMP_SIZE_W-1] == 254 || input_g[j+TMP_SIZE_H-1][i] == 254 || input_g[j+TMP_SIZE_H-1][j+TMP_SIZE_W-1] == 254){
    ...
    */
    ...
    // ラスタスキャン
    for( j = 48; j < loss_SIZE_H-48; j++ ){
        for( i = 30; i < loss_SIZE_W-30; i++ ){
